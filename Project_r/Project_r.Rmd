...
ggplot(monthly_deaths_graph,aes(x=date_new,y=proportion_deaths))+geom_line()+scale_x_date(date_breaks = "1 year")
```
#5.2. Utilice la función labs para dar al eje x y al eje y una estética a las etiquetas.
```{r}
ggplot(monthly_deaths_graph,aes(x=date_new,y=proportion_deaths))+geom_line()+labs(y="Proportion Deaths",x="Date",title="Graph of proportion deaths vs births", subtitle ="Date of analysis: 1841 to 1849",caption="Data of clinic 1" )+scale_x_date(date_breaks = "1 year")
```
...
```{r}
ggplot(death_by_year, aes(x=new_year,y=year_median))+geom_col()+labs(y="Mean",x="Date",title="Graph of mean of proportion deaths", subtitle ="Date of analysis: 1841 to 1849",caption="Data of clinic 1" )+scale_x_continuous(breaks=c(1841:1849))
```
...
ggplot(monthly_deaths_2,aes(x=date_new,y=proportion_deaths,color=color_line))+geom_line()+scale_x_date(date_breaks = "1 year")+theme(axis.text.x =element_text(size =10, angle = 45))
```
...
```{r}
ggplot(monthly_deaths_2,aes(x=date_new,y=proportion_deaths,color=color_line))+geom_line()+labs(title="Proportion deaths", subtitle ="Date of death" )+scale_x_date(date_breaks = "1 year")+theme(axis.text.x =element_text(size = 10, angle = 45))
```
...
```
#Caculo de media arimetica, desviacion estandar y cantidad de elementos.
```{r}
monthly_deaths_sel%>%
group_by(handwashing_started)%>%
...
#EJEMPLO PRUEBA DE HIPOTESIS 2 MUESTRAS DEPENDIENTES T-STUDENT
...
# https://rstudio-pubs-static.s3.amazonaws.com/748718_00d42d8f62f6427fa61107dde777f743.html
